...
//USAGE:target time threads reqs proxyfile GET/PRI refererfile
...
var proxies = fs.readFileSync(proxyfile, 'utf-8').toString().replace(/\r/g, '').split('\x0A');//代理文件
var referers = fs.readFileSync(refererfile, 'utf-8').toString().replace(/\r/g, '').split('\x0A');//HTTP_REFERER
...
//定义了UAS，本人建议多加几UserAgent数量，因为各别CDN会根据UserAgent进行block操作
const UAs = [
